...
class IssueModal extends Component {
  ...
  showModelHandler = (e) => {
    ...
    dispatch({
      ...
      callback(data) {
        dispatch({
          type: 'app/result', payload: { data, namespace }, onHander({ data }) {
            _self.setState({
              visible: true,
              classifications: data
            });
  ...
  render() {
    ...
    return (
      <span>
        ...
        >
          <Form mode='horizontal' onSubmit={this.okHandler}>
            <Row gutter={16}>
              <Col sm={12}>
                <FormItem
                  label="类型"
              ...
              <Col sm={24}>
                ...
                >
                  {getFieldDecorator('digest', {
                    ...
                    <Input type="textarea" placeholder="摘要信息，如果不输入截取文章信息" autosize={{ minRows: 3, maxRows: 6 }}/>
                    )}
                </FormItem>
              </Col>
            </Row>
